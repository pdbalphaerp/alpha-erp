...
class Main3CX(http.Controller):
    ...
    def search_3cx_number(self, number, ttype):
        ...
        if not (partner := request.env['res.partner'].sudo().search(['|', ('phone_format', 'ilike', str(sanitized_number)), ('phone_1_format', 'ilike', str(sanitized_number))], limit=1)):
            partner = request.env['res.partner'].sudo().create({
    ...
    def _3cx_log_call(self):
        ...
        if not (partner := request.env['res.partner'].sudo().search(['|', ('phone_format', 'ilike', str(number)), ('phone_1_format', 'ilike', str(number))], limit=1)):
